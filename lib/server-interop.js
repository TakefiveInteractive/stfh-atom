...
export let createRoom = roomName =>
  ...
  .then(({ roomId, userId }) => {
    ...
    return {
      ...
      updateCursor: (selection, pos) => p('editor:cursor', { roomId, selection, pos }),
      insert: (pos, content) => p('editor:insert', { roomId, pos, content }),
      del: selection => p('editor:delete', { roomId, selection }),
      fileRefreshRequest
    }
  }
);
